...
local RunTshake = io.open("MohamedApi", 'w')
RunTshake:write([[
#!/usr/bin/env bash
cd $HOME/MohamedApi
...
while(true) do
rm -fr ../.telegram-cli
./tg -s ./MohamedApi.lua -p PROFILE --bot=$token
done
]])
RunTshake:close()
local RunTs = io.open("Mo", 'w')
RunTs:write([[
#!/usr/bin/env bash
cd $HOME/MohamedApi
while(true) do
rm -fr ../.telegram-cli
screen -S MohamedApi -X kill
screen -S MohamedApi ./MohamedApi
done
]])
RunTs:close()
end
